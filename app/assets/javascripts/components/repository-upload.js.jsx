    ...
    selectFiles: function (e) {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            (function () {
                ...
                file.timeStart = (new Date().getTime());

                var file_path = (self.props.path_prefix || '') + self.state.path + file.name;
                ...
                evaporate.add({
                    ...
                    complete: function () {
                        ...
                        $.post(self.props.upload_complete_url, {
                            details: {
                                file: file_path,
                                file_size: file.size,
                                bucket: self.props.s3_options.bucket,
                                upload_duration: Math.round(((new Date().getTime()) - file.timeStart) / 1000) + ' seconds'
                            }
                        });
                    ...
                    error: function () {
                        if (self.props.s3_options.url.indexOf(':') == -1) {
                            file.error = true;
                            self.refreshFilesState();
                        }
                        else{ //fakeS3
                            $.post('s3/create_folder', {
                                bucket: self.props.s3_options.bucket,
        ...
        return (
            <div>
                <div className="file-input-text">
                    Drag and drop files here to upload them to current directory (or click here to pick files)
                </div>
